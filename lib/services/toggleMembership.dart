...
class ToggleMemMethods{
  ...
  joinChat(String hashTag, String groupId, String username, String admin) async{
    await DatabaseMethods(uid: userId).toggleGroupMembership(groupId, "JOIN_PUB_GROUP_CHAT");
  ...
  goOnWaitListAndOrSpectate(String groupId, String hashTag, String admin, String groupState) async{
    await DatabaseMethods(uid: userId).putOnWaitList(groupId, Constants.myName, Constants.myUserId, Constants.myEmail, null);
  ...
  requestJoin(
    ...
    if(numOfMem < groupCapacity){
      await DatabaseMethods(uid: userId).requestJoinGroup(
          groupId,
          Constants.myName,
          Constants.myUserId,
          Constants.myEmail,
