...
getFileUrl(String type, File file) async{
  ...
  ref = FirebaseStorage.instance
      .ref()
      .child('$type/${Constants.myUserId}/${DateTime.now().microsecondsSinceEpoch}');
...
fileUploadToChats({
  ...
  String chatId;
  if(personalChatId != null){
    chatId = await DatabaseMethods(uid: Constants.myUserId).uploadFileToPersonalChat(
        personalChatId:personalChatId,
        contactId:contactId,
        imgMap:imgObj,
        fileMap: fileObj,
...
fileUploadToStory({
  ...
  String storyId = await DatabaseMethods(uid: Constants.myUserId).uploadFileToStory(
        mediaObj:imgObj,
        mediaGallery: mediaGallery,
        sendTime:time,
        type:storyType,
