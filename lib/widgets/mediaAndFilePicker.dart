            ...
            Align(
              ...
              child: GestureDetector(
                onTap: () async{
                  ...
                  if(asset.type == AssetType.video)
                    Navigator.push(context, MaterialPageRoute(builder: (context) =>
                        VideoAudioFilePreview(videoFile: file, fullScreen:true, play: true,)
                    ));
                  else
                    Navigator.push(context, MaterialPageRoute(builder: (context) =>
                        PhotoView(
                ...
                child: Padding(
                  ...
                  child: Icon(Icons.open_in_new_rounded, color:Colors.orange,),
                ),
              )
            ),
          ],
...
class FileDisplay extends StatelessWidget {
  ...
  FileDisplay({this.file, this.vidOrAud, this.audioName, this.fileName});
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        SizedBox.expand(
          child: vidOrAud ?
          VideoAudioFilePreview(videoFile: file, fullScreen:false, play: false, audioName: audioName) :
          fileName == null ?
          Image.file(file, fit:BoxFit.cover) :
          filePreview(context, "assets/images/docImage.png", fileName, false),
        ),
        if(vidOrAud)
          audioName == null ?
          videoIcon() :
...
class UnknownFileDisplay extends StatelessWidget {
  final String fileName;
  UnknownFileDisplay(this.fileName);

  @override
  Widget build(BuildContext context) {
    return filePreview(context, "assets/images/unknownFile.png", fileName, false);
  }
}
...
class _MediaAndFileGalleryState extends State<MediaAndFileGallery> {
  ...
  fetchMedia() async{
    final bool permitted = await PhotoManager.requestPermission();
    if(permitted){
      lastPage = currentPage;
      List<AssetPathEntity> albums = await PhotoManager.getAssetPathList(onlyAll: true);
      List<AssetEntity> mediaList = await albums.first.getAssetListPaged(currentPage, 60);
  ...
  void initState() {
    ...
    if(widget.type == "MEDIA"){
      ...
      widget.controller.addListener(() {
        if(widget.controller.position.pixels / widget.controller.position.maxScrollExtent > 0.33)
    ...
    return GestureDetector(
      onTap: () async{
        if(widget.singleFile == null || !widget.singleFile){
          bool moreUplAvl = widget.numOfAvlUpl == null ? numOfSFs < Constants.maxFileUpload :
          ...
          if(moreUplAvl){
            ...
            if(getFileSize(file) <= 10){
              ...
              if(!selected){
                ...
                setState(() {
                  selFiles.add(SelectedFile(asset:asset, uploadTo:widget.uploadTo));
                  numOfSFs++;
                });
              ...
              Fluttertoast.showToast(
                  msg: "Sorry, file size exceeds 10MB",
          ...
          if(getFileSize(file) <= 10){
            Navigator.pushReplacement(
                ...
                MaterialPageRoute(builder: (context) => PreviewScreen(
                  ...
                  tagPublic: widget.personalChatId == null && widget.groupId == null,
  ...
  sendToChat(List<SelectedFile> rdyFile){
    ...
    return Container(
      ...
      child: Column(
        children: [
                ...
                children: [
                  IconButton(
                    icon : Icon(
                        ...
                        platform == TargetPlatform.android ? Icons.send : CupertinoIcons.arrow_up_circle :
                    ...
                    onPressed: (){
                      if(widget.numOfAvlUpl == null)
                        ...
                        uploadedSnip = true;
                        Navigator.pop(context, selFiles.where((m) => !m.deleted).toList());
                      }
                    },
                  ),
                  Text(
                    widget.numOfAvlUpl == null ? "$numOfSFs/${Constants.maxFileUpload}" :
                    "$numOfSFs/${widget.numOfAvlUpl}",
          ...
          Divider(height: 25, thickness: 2.5, color: Colors.black, indent: 90, endIndent: 90,),
          widget.type == "MEDIA" ?
          Expanded(
            ...
            child: Column(
              ...
              children: [
                ...
                numOfSFs < Constants.maxFileUpload ? TextButton.icon(
                    onPressed: () async{
                      ...
                      if(widget.type == "AUDIO")
                        result = await FilePicker.platform.pickFiles(
                            ...
                            allowMultiple: true,
                            allowedExtensions: ['mp3', 'wav', 'wma', 'aac', 'flac', 'm4a'],
                        );
                      else if(widget.type == "PDF")
                        result = await FilePicker.platform.pickFiles(
                          type: FileType.custom,
                      ...
                      if(result != null){
                        List<PlatformFile> files = result.files.length > Constants.maxFileUpload - numOfSFs ?
                        result.files.sublist(0, Constants.maxFileUpload - numOfSFs) : result.files;

                        for(PlatformFile file in files) {
                          if(getFileSize(File(file.path)) <= 15){
                            selFiles.add(SelectedFile(platformFile: file, uploadTo: widget.uploadTo));
                            numOfSFs++;
                          }
                          else
                            Fluttertoast.showToast(
                                msg: "Sorry, "+file.name+" exceeds 15MB",
                                toastLength: Toast.LENGTH_SHORT,
                                gravity: ToastGravity.SNACKBAR,
                                timeInSecForIosWeb: 2,
                                backgroundColor: Colors.black,
                    ...
                    label: Text(
                      widget.type == "AUDIO" ? "select audio files" : "select PDF files",
                      style: TextStyle(
...
class _SelectedFileState extends State<SelectedFile> {
  ...
  fileSetUp() async{
    if(widget.file != null)
      ...
      file = File(widget.platformFile.path);
      audioName = audioChecker(widget.platformFile.name) ? widget.platformFile.name : null;
      fileName = pdfChecker(widget.platformFile.name) ? widget.platformFile.name : null;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () async{
        if(widget.platformFile == null || audioChecker(widget.platformFile.name) || pdfChecker(widget.platformFile.name)){
          List res = await Navigator.push(context, MaterialPageRoute(
      ...
      child: ClipRRect(
        ...
        child: file != null ? Stack(
          children: [
            ...
            widget.file != null ||
                (widget.platformFile != null && (audioChecker(widget.platformFile.name) || pdfChecker(widget.platformFile.name))) ?
            FileDisplay(file:file, vidOrAud:isVidOrAud, audioName: audioName, fileName: fileName) :
            ...
            Align(
                ...
                child: Icon(Icons.gif_rounded, size: 27, color: Colors.orange,)
            ) : SizedBox.shrink(),
