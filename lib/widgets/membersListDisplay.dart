    ...
    child: Column(
      children: [
        ...
        GestureDetector(
          onTap: ()async{
            if(editGroup){
              String actionType = await showBanMemberDialog(context, hashTag, userId, anon);
              DatabaseMethods(uid: userId).toggleGroupMembership(groupId,  actionType);
            }
          },
          child: Container(
              ...
              child: Row(
                children: [
                  Text(
                      editGroup ? "Ban" : admin ? "Admin" : "Mem",
                      style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)
                  ),
                  !editGroup && admin ? Container(
              ...
              if(snapshot.data.docs.length > 0){
                return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      String userId = snapshot.data.docs[index].data()['userId'];
                      if(edit){
                        print(snapshot.data.docs.length);
                        return snapshot.data.docs.length > 1 ?
                        userId != Constants.myUserId && (blockList == null || !blockList.contains(userId)) ?
                        memberTile(
                            userId: userId,
                            context: context,
                            editGroup: edit,
