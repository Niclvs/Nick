...
class _SharedChatBubbleState extends State<SharedChatBubble> {
  ...
  getOgSenderInfo() async{
    DocumentSnapshot userDS = await DatabaseMethods(uid: ogSenderId).getUserById();
    if(mounted && userDS.exists)
      setState(() {
        ogSender = userDS.data()["name"];
        profileImg = anon == null || !anon ? userDS.data()["profileImg"] : userMIYUs[userDS.data()["anonImg"]] ;
      });
  }

  setUp(){
    if(widget.imgObj != null || widget.mediaGallery != null){
      Map imgObj = widget.imgObj != null ? widget.imgObj : widget.mediaGallery[0];
      setState(() {
        anon = imgObj['anon'];
      ...
      if(imgObj["ogStoryId"] != null){
        ...
        DatabaseMethods(uid: Constants.myUserId).markSeenStory(ogSenderId, mediaId);
      }else{
        setState(() {
            ...
            children: [
              ...
              Align(
                ...
                child: Container(
                  color: Colors.white,
                  child: ListTile(
                    leading: profileImg != null ? GestureDetector(
                      onTap: (){
                        if(anon == null || !anon){
                          Navigator.push(context, MaterialPageRoute(
                              builder: (context) => UserProfileScreen(userId:ogSenderId,)
                          ));
                        }
                      },
                      child: avatarImg(profileImg, 24),
                    ) : SizedBox.shrink(),
                    title: Text(
                        ogSenderId == Constants.myUserId ? "Me" :
                        anon == null || !anon ? ogSender :
                        "Anonymous",
                        style: TextStyle(
                            color: Colors.black,
...
Widget unknownFileBubble(Map fileObj, bool isSendByMe, platform){
  return ListTile(
    leading: avatarImg("assets/images/unknownFile.png", 24),
    title: Text(
        fileObj["fileName"],
        style: TextStyle(
            color: Colors.white,
    ...
    dense: true,
    trailing: fileObj['fileUrl'] != null ? Icon(
      ...
      child: CircularProgressIndicator(
          valueColor: AlwaysStoppedAnimation<Color>(isSendByMe ? Colors.black54 : Colors.purple)
      ),
    ),
    onTap: ()async{
      if(fileObj['fileUrl'] != null){
        ...
        if(ready){
          ...
          String taskId = await DownloadMethods.startDownload(fileObj['fileName'], fileObj['fileUrl'], savedDir);
          if(taskId != null){
...
Widget groupTextBubble(BuildContext context, String message, List highlightWords, bool isSendByMe){
...
Widget replyMediaBubble({
  ...
  return imgObj != null || (fileObj != null && (audioChecker(fileObj['fileName']) || pdfChecker(fileObj['fileName']))) ?
  Container(
