...
Widget mediaAndFileTile({
  ...
  Widget tile = Container(
      decoration: imgObj != null && imgObj["sticker"] != null && imgObj["sticker"] ? null : shadowEffect(30),
...
Widget infoListTile(
  ...
  return Container(
    ...
    child: ListTile(
      ...
      subtitle: userName(userId: senderId, anon: anon, fontWeight: FontWeight.bold),
      trailing: StreamBuilder(
        stream: DatabaseMethods().groupChatCollection.doc(groupId).collection('chats').doc(messageId).snapshots(),
        builder: (context, snapshot) {
          ...
          GestureDetector(
            onTap: () {
              showRepliedUsersDialog(replies, messageId, groupId, context, anon != null && anon);
            },
            child: Container(
          ...
          ) : Constants.myUserId != senderId && replied != null && !replied ?
          IconButton(
            icon: Icon(
            Icons.maps_ugc,
            color: Colors.black
    ...
    children: [
                      ...
                      Row(
                        children: [
                          IconButton(
                              ...
                              color: Colors.black,
                              iconSize: platform == TargetPlatform.android ? 25 : 20,
                              onPressed: () {
                  ...
                  Row(
                    children: [
                      StreamBuilder(
                          ...
                          builder: (context, snapshot) {
                            if(snapshot.hasData){
                              ...
                              return IconButton(
                                  icon: Icon(!saved ? Icons.bookmark_border : Icons.bookmark),
                                  color: Colors.black,
                                  iconSize: platform == TargetPlatform.android ? 25 : 20,
                                  onPressed: () async{
                                    if(!saved){
                      ...
                      StreamBuilder(
                        ...
                        builder: (context, snapshot) {
                          return IconButton(
                              icon: Icon(platform == TargetPlatform.android ? Icons.more_horiz : CupertinoIcons.ellipsis),
                              color: Colors.black,
                              iconSize: platform == TargetPlatform.android ? 25 : 20,
                              onPressed: () {
                                openMoreOpsBttSheet(
      ...
      Align(
          ...
          child: Padding(
            ...
            child: mediaCommentList(context:context, mediaId:messageId, heightDiv: 0.15, anon: anon),
          )
      )
    ],
  );
