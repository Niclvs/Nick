      ...
      children: [
        ...
        Expanded(
          child: Form(
            ...
            child: TextFormField(
                validator: (val){
                  return !emptyStrChecker(val) ? val.length <= 300 ? null :
                ...
                decoration: msgInputDec(context:context, hintText: "Reply")
            ),
          ),
        ),
        IconButton(
          ...
          color: Colors.orange,
          onPressed: () async{
            await addReply(reFormKey, replyEditingController, storyId, commentId);
...
Widget replyList(ScrollController controller, Stream replyStream, String storyId, String commentId){
        ...
        builder: (context, snapshot){
          ...
          ListView.builder(
            ...
            itemBuilder: (BuildContext context, index){
              String replierId = snapshot.data.docs[index].data()["replierId"];
          ...
          ) : noItems(icon:Icons.reply, text: "no replies yet", mAxAlign: MainAxisAlignment.center);
        },
      );
    }
  );
