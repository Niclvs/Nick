  ...
  return Column(
    children: [
      GestureDetector(
          ...
          child: groupProfile(groupId:groupId, oneDay:oneDay, timeElapsed:timeElapsed, profileImg:profileImg),
      ),
...
Widget userIcon(
  ...
  sendBy = senderId == Constants.myUserId ? "Me" : anon != null && anon ? "Anonymous" : sendBy;
  return GestureDetector(
    ...
    child: Column(
      children: [
        avatarImg(anon != null && anon ? anonImg : profileImg, 24),
        SizedBox(height: 5),
  ...
  return Container(
      ...
      child: RichText(
          ...
          text:TextSpan(
              children: [
                WidgetSpan(
                  child: blocked ? Icon(Icons.block_rounded, color: Colors.red,) : SizedBox.shrink()
                ),
                TextSpan(
                    text: sendBy,
    ...
    children: [
      Container(
        ...
        child: Row(
          ...
          children: [
            mediaCommentList(context:context, mediaId:mediaId, heightDiv: heightDiv, anon: anon),
            senderId != null ?
            Flexible(child: userIcon(context, senderId, sendBy, profileImg, anonImg, anon, blocked)) :
            SizedBox.shrink()
          ],
        ),
      ),
      MediaCommentComposer(mediaId:mediaId, autoFocus: false, fillColor: fillColor, disabled: disabled,)
    ],
  );
}
...
Widget imgToChatBtt(
  ...
  return GestureDetector(
      ...
      child: mediaFuncBtt(groupState == 'private' && !isMember ? Icons.lock_rounded : Icons.remove_red_eye_rounded)
  );
}
      ...
      onTap: (){
        Navigator.push(context, MaterialPageRoute(
            builder: (context) => SendMediaScreen(
              ogSenderId:senderId,
              imgObj:mediaObj != null && mediaObj["imgName"] != null ? mediaObj : null,
              fileObj:mediaObj != null && mediaObj["fileName"] != null ? mediaObj : null,
              messageId:mediaId,
              groupId:groupId,
            ...
            onTap: ()async{
              if(!saved){
                await DatabaseMethods(uid: Constants.myUserId)
                    .saveMedia(imgObj, fileObj, mediaGallery, groupId, senderId, messageId, anon);
                showCenterFlash(alignment: Alignment.center, context: context, text: 'Saved');
              }else{
                await DatabaseMethods(uid: Constants.myUserId)
                    .removeSavedMedia(messageId);
                showCenterFlash(alignment: Alignment.center, context: context, text: 'Unsaved');
              }
            },
            child: mediaFuncBtt(!saved ? Icons.bookmark_border : Icons.bookmark_rounded)
        );
      }else{
...
Widget shareMediaBtt(
  ...
  return GestureDetector(
      onTap: (){
        shareMediaFile(context:context, mediaGallery:mediaGallery, imgObj:mediaObj);
      },
      child: mediaFuncBtt(platform == TargetPlatform.android ? Icons.share_rounded : CupertinoIcons.share)
  );
}
  ...
  return StreamBuilder(
    ...
    builder: (context, snapshot) {
      return GestureDetector(
        ...
        child: mediaFuncBtt(platform == TargetPlatform.android ? Icons.more_horiz : CupertinoIcons.ellipsis),
      );
    }
  );
}
