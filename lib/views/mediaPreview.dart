...
import 'dart:ui';
import 'package:SpidrApp/helper/globals.dart';
...
Widget auxiliaryDisplay({
  ...
  return Stack(
    children: [
      ...
      linkIndicator(context:context, link:link, displayLink: displayLink, topPadding: topPadding) :
      SizedBox.shrink(),
...
class _VideoAudioFilePreviewState extends State<VideoAudioFilePreview> {
  ...
  @override
  void initState(){
    _controller = VideoPlayerController.file(widget.filePath != null ? File(widget.filePath) : widget.videoFile,);
      ...
      child: Stack(
        children: [
          _controller.value.isInitialized ? SizedBox(
            ...
            FittedBox(
              fit: widget.fullScreen != null && !widget.fullScreen ? BoxFit.cover : BoxFit.contain,
              child: SizedBox(
                  width: _controller.value.size?.width ?? 0,
                  height: _controller.value.size?.height ?? 0,
                  child: VideoPlayer(_controller)
              ),
            ) : filePreview(context, "assets/images/audiofile.png", widget.audioName, widget.fullScreen),
...
class _VideoAudioUrlPreviewState extends State<VideoAudioUrlPreview> with TickerProviderStateMixin {
  VideoPlayerController _controller;
  ...
  setUpPlayer(){
    if(mounted){
      if(!widget.video) setState(() {audioName = widget.audioName;});
  ...
  Widget muteButton(){
    return Padding(
      padding: widget.muteBttPadding != null ? widget.muteBttPadding : EdgeInsets.all(9.0),
      child: Align(
