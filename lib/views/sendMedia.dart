...
class _SendMediaScreenState extends State<SendMediaScreen> {
  ...
  shareToChats() async{
    ...
    DatabaseMethods().addFileCopies(mediaId: mediaId, numC: fdChats.length+gcChats.length);
      ...
      builder: (context, snapshot) {
        ...
        ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              String personalChatId = snapshot.data.docs[index].data()["personalChatId"];
              return !Constants.myBlockList.contains(friendId) ? CheckboxListTile(
                contentPadding: EdgeInsets.zero,
                title: userName(userId: friendId, fontWeight: FontWeight.normal, fontSize: 12),
                secondary: userProfile(userId: friendId),
                value: fdChats.containsKey(friendId),
                onChanged:(bool value){
                  if(value){
                    setState(() {
                      fdChats[friendId] = {'personalChatId':personalChatId};
                    });
                  }else{
                    setState(() {
      ...
      builder: (context, snapshot) {
        ...
        ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              CheckboxListTile(
                contentPadding: EdgeInsets.zero,
                title: Text(hashTag, style: TextStyle(fontSize: 12)),
                secondary:avatarImg(profileImg, 24),
  ...
  void initState() {
    ...
    super.initState();
    setState(() {
      mediaId = widget.messageId != null ? widget.messageId : widget.storyId;
    });
    getGCChats();
    getFdChats();
  }
        ...
        children: [
          ...
          AlertDialog(
            ...
            actions: [
              ...
              TextButton(
                onPressed:(){
                  if(fdChats.length > 0 || gcChats.length > 0){
                    shareToChats();
                    showCenterFlash(alignment: Alignment.center, context: context, text: 'Sent');
                  }
                  Navigator.pop(context);
                },
