...
import 'package:SpidrApp/views/preview.dart';
...
class AppCameraScreen extends StatefulWidget {
  ...
  static const double CAMERA_BUTTON_SIZE = 80;
...
class _AppCameraScreenState extends State<AppCameraScreen> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  int numOfSMs = 0;
  ScrollController selController = new ScrollController();
    ...
    return Align(
      ...
      child: IconButton(
        onPressed: () {
          ...
          CameraMethods.onSwitchCamera(
              cameras, newCameraIndex, initCamera);
          setState(() {
  ...
  void takePicture() async {
    if(!onCapture){
      ...
      if(multiMedia){
        File imgFile = await CameraMethods.onCaptureForMulti(context, cameraController);
        if(numOfSMs < Constants.maxFileUpload && imgFile != null){
          String uploadTo = widget.groupChatId == null && widget.personalChatId == null ? "SNIPPET" :
          widget.groupChatId != null ? "GROUP" :
          "PERSONAL";

          setState(() {
            selMedia.add(SelectedFile(file: imgFile, uploadTo: uploadTo, video: false));
            numOfSMs++;
          });
          Timer(
            Duration(seconds: 1),
  ...
  void takeVideo() async {
    if (cameraController.value.isRecordingVideo)
      return null; //only proceed if not already recording

    filepath = await CameraMethods.onCaptureVideo(context, cameraController);

    setState(() {
      _recordingProgressAnimation = "Demo"; //setting to the name of the animation will trigger it to start
    });
    ...
    Timer.periodic(new Duration(milliseconds: 10), (timer) {
      if (timer.tick >= AppCameraScreen.MAX_VIDEO_DURATION *
          100) { // max recording time is 7 seconds, 700 durations of 10 milliseconds = 7 seconds
        stopVideo();
      }
      ...
      setState(() {
        percentOfMaxVideoDurationRecorded = timer.tick / (AppCameraScreen.MAX_VIDEO_DURATION * 100);
      });
    });
  }
  ...
  void stopVideo() async{
    ...
    if (cameraController.value.isRecordingVideo) {
      if(multiMedia){
        File videoFile = await CameraMethods.stopVideoForMulti(context, cameraController, filepath);
        if(numOfSMs < Constants.maxFileUpload && videoFile != null){
          ...
          setState(() {
            selMedia.add(SelectedFile(file: videoFile, uploadTo: uploadTo, video: true));
            numOfSMs++;
          });
    ...
    setState(() {
      _recordingProgressAnimation = "Idle"; //setting to a name that does not refer to an actual animation will stop it
    });
  }
    ...
    return SizeTransition(
      ...
      child: ClipRect(
        child: Row(
          ...
          children: [
            IconButton(
              icon: Icon(platform == TargetPlatform.android ? Icons.flash_off : CupertinoIcons.bolt_slash_fill),
            ...
            IconButton(
              icon: Icon(platform == TargetPlatform.android ? Icons.flash_auto : CupertinoIcons.bolt_badge_a_fill),
            ...
            IconButton(
              icon: Icon(platform == TargetPlatform.android ? Icons.flash_on : CupertinoIcons.bolt_fill),
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Center(
                child: Text("Exposure Offset",style: TextStyle(color:Colors.white)),
              ),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                mainAxisSize: MainAxisSize.max,
    ...
    return Container(
      ...
      child: numOfSMs > 0 ? ListView.builder(
          ...
          itemBuilder:(context, index){
            return !selMedia[index].deleted ? Padding(
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Expanded(child: Divider(height: 25, thickness: 1.5, color: Colors.white54, indent: 45,)),
                                  Text("$numOfSMs/${Constants.maxFileUpload}", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),),
                                  Expanded(child:Divider(height: 25, thickness: 1.5, color: Colors.white54, endIndent: 45,)),
                                ],
                              ),
                            ],
                          ),
                          selectedMediaList(),
                          Divider(height: 15, thickness: 1.5, color: Colors.white54, indent: 45, endIndent: 45,),
                        ],
                      ) : SizedBox.shrink(),
                          ...
                          children: [
                            ...
                            Align(
                              ...
                              child: IconButton(
                                icon: multiMedia ?
                                Icon(
                                    numOfSMs < Constants.maxFileUpload ? Icons.upload_rounded : Icons.upload_outlined,
                                    color: numOfSMs < Constants.maxFileUpload ? Colors.white : Colors.white54
                                ) : Icon(
                                ...
                                onPressed: () async{
                                  ...
                                  if(multiMedia){
                                    if(numOfSMs < Constants.maxFileUpload){
                                      List<SelectedFile> localMedia = await openUploadBttSheet(
                                          context: context,
                                          uploadTo: uploadTo,
                                          numOfAvlUpl: Constants.maxFileUpload-numOfSMs
                                      );
            ...
            Container(
              ...
              child: Align(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    widget.groupChatId == null && widget.personalChatId == null ? Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.settings_input_antenna_rounded, color: Colors.orange,),
                      ],
                    ) : SizedBox.shrink(),
                    Row(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              icon: Icon(platform == TargetPlatform.android ? adFlashIcon : iosFlashIcon),
                              color: Colors.white,
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          icon: Icon(!multiMedia ? Icons.photo : Icons.collections_rounded, color: Colors.white,),
                          onPressed: cameraController != null ? onMultiSnippetModeButtonPressed : null,
                        )
                      ],
                    ),
                    Row(
