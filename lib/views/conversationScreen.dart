...
import 'package:SpidrApp/services/toggleMembership.dart';
...
class _ConversationScreenState extends State<ConversationScreen>
  ...
  getMessages(){
    if(mounted){
      setState(() {
        chatMessageStream = DatabaseMethods(uid: widget.uid).getConversationMessages(widget.groupChatId);
      });
    }
  }
  ...
  sendMessage({Map inChatReply}){
    if(!emptyStrChecker(messageController.text)){
      ...
      String ogSenderId;
      if(inChatReply != null){
        if(inChatReply["msgReplyTo"] is List && inChatReply["msgReplyTo"][0]["ogSenderId"] != null){
          ogSenderId = inChatReply["msgReplyTo"][0]["ogSenderId"];
        }else if(inChatReply["msgReplyTo"] is Map && inChatReply["msgReplyTo"]["ogSenderId"] != null){
      ...
      if(inChatReply != null){
        if(inChatReply["msgReplyTo"] is Map || inChatReply["msgReplyTo"] is List){
  ...
  deleteMessage(String chatId, Map imgObj, Map fileObj, List mediaGallery, int msgIndex, String message){

    DatabaseMethods(uid: widget.uid).deleteConversationMessage(groupChatId:widget.groupChatId, chatId:chatId);
  ...
  Widget messageTile(
    ...
    Widget mediaCommentBtt = IconButton(
      icon: Icon(Icons.add_comment_rounded, color: Colors.orange),
      onPressed: (){
    ...
    return !blocked ? Column(
      children: [
        ...
        Row(
          children: [
            Container(
              margin: EdgeInsets.fromLTRB(9, 0.0, 2.5, 2.5),
              child: userProfile(userId:userId, anon:anon != null && anon)
            ),
            anon == null || !anon ? RichText(
              text: TextSpan(
                  children: [
                    TextSpan(
                        text: sendBy+" ",
                    ...
                    userId == admin ?
                    WidgetSpan(
                      child: Icon(!isSendByMe ? Icons.circle : null, size: 14, color: Colors.orange,),
                    ) : TextSpan(),
                  ]
              ),
            ) : SizedBox.shrink()
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              !isSendByMe ?
              Center(child: Text(hourMin, style: TextStyle(color: Colors.grey),),) :
              SizedBox.shrink(),

              GestureDetector(
                onLongPress: () async{
                  if(message.isNotEmpty || (imgObj != null && imgObj["imgUrl"] != null) ||
                      fileObj != null && fileObj["fileUrl"] != null ||
                      (mediaGallery != null && mediaGallery[0]["imgUrl"] != null)){

                    DocumentSnapshot mdDS = await DatabaseMethods().mediaCollection.doc(messageId).get();

                    isSendByMe ? showMenu(
                        context: context,
                        position: RelativeRect.fromLTRB(0.0, MediaQuery.of(context).size.height, 0.0, 0.0),
                        items: <PopupMenuEntry>[
                          (imgObj != null && imgObj["ogSenderId"] == null) ||
                              (mediaGallery != null && mediaGallery[0]["ogSenderId"] == null) ||
                              (fileObj != null && (pdfChecker(fileObj["fileName"]) || audioChecker(fileObj["fileName"]))) ?
                          PopupMenuItem(
                              value:2,
                              child: iconText(
                                  mdDS.exists ? Icons.explore_off_rounded : Icons.explore_rounded,
                                  mdDS.exists ? " Hide" : " Post"
                              )
                          ) : null,
                          PopupMenuItem(
                              value:1,
                              child: iconText(
                                  numOfReplies == 0 ? CupertinoIcons.delete : Icons.open_in_new,
                                  numOfReplies == 0 ? " Delete" : " Open Replies"
                              )
                          ),
                        ]).then((value) {
                      if (value == 1){
                        if(numOfReplies == 0){
                          String deleteMsg;
                          if(message.isNotEmpty) deleteMsg = message.toLowerCase();
                          else{
                            if(imgObj != null){
                              if(imgObj['caption'] != null && imgObj['caption'].isNotEmpty){
                                deleteMsg = imgObj['caption'].toLowerCase();
                              }
                            }
                          }
                          deleteMessage(messageId, imgObj, fileObj, mediaGallery, index, deleteMsg);
                        }else{
                          showRepliedUsersDialog(replies, messageId, widget.groupChatId, context, anon != null && anon);
                        }
                      }else if(value == 2){
                        if(mdDS.exists){
                          ...
                          addMediaItem(
                              ...
                              mediaId:messageId,
                              sendTime:time,
                              mediaObj:imgObj != null ? imgObj : fileObj,
                      ...
                      if(value == 1){
                        ...
                        String msg = message.isNotEmpty ? message :
                        imgObj != null ? imgObj["imgName"] :
                        fileObj != null ? fileObj["fileName"] :
                        mediaGallery.map((e) => e["imgName"]).toList().join(' ');

                        setState(() {
                          msgReplyTo = msg;
                          replyInfo = {
                            "userReplyTo":userId,
                            "msgReplyTo":message.isNotEmpty ? message :
                            imgObj != null ? imgObj :
                            fileObj != null ? fileObj :
                            mediaGallery,
                            "msgId": ogMediaId != null ? ogMediaId : messageId
                          };
                        });
                      }else if(value == 3){
                        reportContent(
                ...
                child: Container(
                  ...
                  child: Container(
                      ...
                      decoration: chatBubbleDec(
                          isSendByMe,
                          message.isNotEmpty || (fileObj != null && !audioChecker(fileObj["fileName"]) && !pdfChecker(fileObj["fileName"]))
                      ),

                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
                          inChatReply != null ? Container(
                            ...
                            child: Column(
                                ...
                                Container(
                                  margin: EdgeInsets.fromLTRB(2.5, 0.0, 0.0, 2.5),
                                  child: Row(
                                    children: [
                                      Icon(Icons.reply_rounded, color: isSendByMe ? Colors.black : Colors.orange,),
                                      SizedBox(width: 2.5,),
                                      userProfile(userId: inChatReply["userReplyTo"], anon: anon != null && anon, size: 18),
                                      SizedBox(width: 2.5,),
                                      anon == null || !anon ?
                                      userName(
                                          userId: inChatReply["userReplyTo"],
                                          fontWeight: FontWeight.bold,
                                ...
                                inChatReply["msgReplyTo"] is String ?
                                replyTextBubble(context, inChatReply["msgReplyTo"], isSendByMe) :
                                inChatReply["msgReplyTo"] is Map ? inChatReply["msgReplyTo"]["ogSenderId"] != null ?
                                SharedChatBubble(
                                  imgObj: inChatReply["msgReplyTo"]['imgName'] != null ? inChatReply["msgReplyTo"] : null,
                                  fileObj: inChatReply["msgReplyTo"]['fileName'] != null ? inChatReply["msgReplyTo"] : null,
                                  isSendByMe: isSendByMe,
                                  mediaId: inChatReply["msgReplyTo"]['fileName'] != null ? inChatReply["msgReplyTo"]["ogChatId"] :
                                  inChatReply["msgReplyTo"]["ogChatId"] != null ?
                                  inChatReply["msgReplyTo"]["ogChatId"] :
                                  inChatReply["msgReplyTo"]["ogStoryId"],
                                  reply: true,
                                ) : replyMediaBubble(
                                    context:context,
                                    imgObj:inChatReply["msgReplyTo"]['imgName'] != null ? inChatReply["msgReplyTo"] : null,
                                    ...
                                    isSendByMe:isSendByMe,
                                    platform: platform,
                                    senderId: inChatReply["userReplyTo"]
                                ) : inChatReply["msgReplyTo"][0]["ogSenderId"] != null ?
                                  SharedChatBubble(
                                    mediaGallery: inChatReply["msgReplyTo"],
                                    isSendByMe: isSendByMe,
                                    mediaId: inChatReply["msgReplyTo"][0]["ogChatId"] != null ?
                                    inChatReply["msgReplyTo"][0]["ogChatId"] :
                                    inChatReply["msgReplyTo"][0]["ogStoryId"],
                                    reply: true,
                                  ) : MediaGalleryBubble(
                                  mediaGallery:inChatReply["msgReplyTo"],
                                  messageId:messageId,
                          ...
                          message.isNotEmpty ?
                          groupTextBubble(context, message, highlightWords, isSendByMe) :
                          ...
                          SharedChatBubble(
                            ...
                            children: [
                              fileChatBubble(
                                  ...
                                  isSendByMe:isSendByMe,
                                  platform:platform,
                                  audio: audioChecker(fileObj["fileName"]),
                                  document:pdfChecker(fileObj["fileName"]),
                                  senderId: userId,
                                  groupId: widget.groupChatId,
                                  hashTag: hashTag,
                                  anon: anon,
                              ),
                              audioChecker(fileObj["fileName"]) || pdfChecker(fileObj["fileName"]) ?
                              mediaCommentBtt :
                              SizedBox.shrink()
                            ],
                          ) :
                          ...
                          SharedChatBubble(
                            imgObj: imgObj,
                            isSendByMe: isSendByMe,
                            mediaId: imgObj["ogChatId"] != null ? imgObj["ogChatId"] :
                            imgObj["ogStoryId"]
                          ) : Column(
                            crossAxisAlignment:CrossAxisAlignment.start,
                            children: [
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                numOfReplies > 1 ?
                                numOfReplies.toString() + " users replied" :
                                numOfReplies.toString() + " user replied",
      ...
      builder: (context, snapshot){
        ...
        ScrollablePositionedList.builder(
            ...
            itemBuilder: (context, index) {
              ...
              bool reported = reportedBy != null && reportedBy.contains(Constants.myUserId);
              ...
              if(index > 0){
                int prevSendTime = snapshot.data.docs[index - 1].data()["time"];
                String preSendDateTime = timeToString(prevSendTime);
              ...
              messageTile(
                ...
                newDay,
                index,
                snapshot.data.docs[index].data()['inChatReply'],
                snapshot.data.docs[index].data()['ogMediaId'],
                reported,
                snapshot.data.docs[index].data()['ogSenderId'],
              ) : SizedBox.shrink();
  ...
  Widget buildMsgCompAndMemTog(platform, bool isOnWaitList, bool reqJoin, bool gotBanned, bool isMember, bool expired){
    return Container(
      height: 54.0,
      color: Colors.white,
      child: isMember != null && isMember ? Container(
      ...
      GestureDetector(
            ...
            if(groupState == 'private'){
              if(!isOnWaitList && !reqJoin && !loading){
                ...
                if(numOfMem < groupCapacity){
                  await ToggleMemMethods(userId: widget.uid, context: context)
                      .requestJoin(widget.groupChatId, numOfMem, groupCapacity, groupState, hashTag, admin);
                }else{
                  await ToggleMemMethods(userId: widget.uid, context: context)
                      .goOnWaitListAndOrSpectate(widget.groupChatId, hashTag, admin, groupState);
                }
                setState(() {
                  loading = false;
                });
              ...
              if(!isOnWaitList && !isMyChat && !loading){
                ...
                if(numOfMem < groupCapacity){
                  await ToggleMemMethods(userId: widget.uid, context: context)
                      .joinChat(hashTag, widget.groupChatId, Constants.myName, admin);
                }else{
                  await ToggleMemMethods(context: context, userId: widget.uid)
                      .goOnWaitListAndOrSpectate(widget.groupChatId, hashTag, admin, groupState);
                }
        ...
        child: Center(
          child: Container(
            decoration: BoxDecoration(
                border: isOnWaitList ? Border.all(
                    color: Colors.redAccent,
                    width: 3.0
                ) : null,
                color: gotBanned ? Colors.black54 :
                !isOnWaitList ? !reqJoin ? numOfMem == groupCapacity ?
                Colors.redAccent : Colors.blue : Colors.grey : null,
            ...
            child: Text(
                ...
                groupState == "public" ? "Join" : "Request" : "Requested" :
                groupState == 'private' ? "Waitlisted" : "Spectating",
                style: TextStyle(
                    color: isOnWaitList ? Colors.redAccent : Colors.white,
  ...
  leaveGroupChat(){
    DatabaseMethods(uid: widget.uid).toggleGroupMembership(widget.groupChatId, "LEAVE_GROUP");
    // Navigator.of(context).pop();
  }

  quitSpectating(){
    DatabaseMethods(uid: widget.uid).toggleGroupMembership(widget.groupChatId, "QUIT_SPECTATING");
    // Navigator.of(context, rootNavigator: true).pop();
  }

  tryJoining() async{
    DocumentSnapshot groupSnapshot = await DatabaseMethods().getGroupChatById(widget.groupChatId);
    int numOfMem = groupSnapshot.data()['members'].length;
    double groupCap = groupSnapshot.data()['groupCapacity'];
    if(numOfMem < groupCap){
      await DatabaseMethods(uid: widget.uid).toggleGroupMembership(widget.groupChatId, "QUIT_SPECTATING");
      DatabaseMethods(uid: widget.uid).toggleGroupMembership(widget.groupChatId, "JOIN_PUB_GROUP_CHAT");
      // Navigator.of(context, rootNavigator: true).pop();
    }else{
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
              ...
              child: Icon(Icons.close, color:Colors.black, size: 13.5)
            ),
        ],
      ),
    );
        ...
        itemBuilder: (context, index){
          return Padding(
            ...
            child: Stack(
              ...
              children: [
                userProfile(userId: memList[index], anon: anon != null && anon, size: 18),
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      stream: DatabaseMethods().groupChatCollection.doc(widget.groupChatId).snapshots(),
      builder: (context, snapshot) {
        if(snapshot.hasData && snapshot.data != null){
          ...
          if(snapshot.data.data() != null &&
            ...
            isMyChat = groupSnapshot.data()['members'].contains(Constants.myUserId);
            ...
            memList = writingMem != null ? writingMem.where((userId) => userId != Constants.myUserId).toList() : [];

            oneDay = groupSnapshot.data()['oneDay'] != null && groupSnapshot.data()['oneDay'];
            ...
            gotBanned = bannedUsers != null && bannedUsers.contains(Constants.myUserId);
            ...
            appBar: AppBar(
                ...
                actions: !disabled ? [
                  ...
                  IconButton(
                    ...
                    onPressed: (){
                      Navigator.push(context, MaterialPageRoute(
                          builder: (context) => AddAndInviteUserScreen(
                              widget.groupChatId,
                              widget.uid,
                              hashTag,
                              true,
                  ...
                  IconButton(
                    ...
                    color: Colors.black,
                    onPressed: () async{
                      bool deleted = await Navigator.push(context, MaterialPageRoute(
                      ...
                      if(deleted != null && deleted) Navigator.pop(context);
                    },
                  ) : SizedBox.shrink(),

                  admin != widget.uid && isMyChat ?
                  PopupMenuButton(
                      icon:Icon(platform == TargetPlatform.android ? Icons.more_vert : CupertinoIcons.ellipsis_vertical, color: Colors.black,),
                      itemBuilder: (BuildContext context) =>
                      [
                        PopupMenuItem(
                            value: !widget.spectate ?  "Invite Friends" : "Become a member",
                            child: Text(!widget.spectate ? "Invite friends" : "Become a member",)
                        ),
                        PopupMenuItem(
                            value:!widget.spectate ? "Leave "+hashTag : "Quit spectating",
                            child: Text(
                                !widget.spectate ? "Leave "+hashTag : "Quit spectating",
                ...
                title: !disabled ? GestureDetector(
                  onTap: (){
                    Navigator.push(context, MaterialPageRoute(
                  ...
                  child: RichText(
                    ...
                    text: TextSpan(
                        children: [
                          ...
                          WidgetSpan(
                            child: Image.asset("assets/icon/icons8-anonymous-mask-50.png", scale: 3.0),
                          ) : TextSpan(),
                        ]
                    ),
                  ),
                ...
                GestureDetector(
                  ...
                  child: Column(
                    children: [
                      ...
                      buildMsgCompAndMemTog(platform, isOnWaitList, reqJoin, gotBanned, isMyChat, disabled),
                    ],
                  ),
                ),
                loading ?
