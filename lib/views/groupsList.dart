...
import 'package:SpidrApp/views/groupChatsView.dart';
...
class _GroupsListState extends State<GroupsList> {
  ...
  Widget searchGroupTile({
    ...
    if(expired && recent) DatabaseMethods(uid: Constants.myUserId).removeRecentSearch("reGroupSearch", groupId);

    return !expired ? GestureDetector(
      onTap: (){
        if(!recent) DatabaseMethods(uid: Constants.myUserId).addRecentSearch("reGroupSearch", groupId);
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    groupStateIndicator(chatRoomState, anon, MainAxisAlignment.start),
              ...
              GestureDetector(
                  onTap: () {
                    if(!recent) DatabaseMethods(uid: Constants.myUserId).addRecentSearch("reGroupSearch", groupId);
                    Navigator.push(context, MaterialPageRoute(
                          builder: (context) => GroupChatsScreen(Constants.myUserId, index, snapshot)
                      ));
                  },
                  child: iconContainer(
          ...
          builder: (context, snapshot) {
            if(snapshot.hasData){
              if(snapshot.data.data() != null){
                ...
                DatabaseMethods(uid: Constants.myUserId).removeRecentSearch("reGroupSearch", groupId);
                return SizedBox.shrink();
              }
            }else{
              return SizedBox.shrink();
