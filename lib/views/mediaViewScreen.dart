...
import 'package:SpidrApp/services/fileDownload.dart';
import 'package:SpidrApp/views/docViewScreen.dart';
import 'package:SpidrApp/widgets/mediaGalleryWidgets.dart';

import 'package:SpidrApp/widgets/storyCommentsWidgets.dart';
...
class _MediaViewScreenState extends State<MediaViewScreen> {
  ...
  startDownload(TargetPlatform platform, String url, String fileName)async{
    bool ready = await checkPermission(platform);
    if(ready){
  ...
  checkStoryExist() async{
    DocumentSnapshot stDS = await DatabaseMethods().userCollection
        .doc(widget.senderId)
        .collection('stories')
        .doc(widget.mediaId)
        .get();
  ...
  Widget exploreMediaBtt(){
    return StreamBuilder(
      stream: DatabaseMethods().mediaCollection.doc(widget.mediaId).snapshots(),
            ...
            onTap: (){
              if(inDiscover)
                ...
                addMediaItem(
                  ...
                  sendTime:sendTime,
                  mediaObj:widget.mediaGallery == null ? widget.mediaObj : null,
                  mediaGallery:widget.mediaGallery
                );
            },
            child: mediaFuncBtt(inDiscover ? Icons.explore_rounded : Icons.explore_off_rounded)
        );
      }
    );
  }
  ...
  Widget storyInfoPanel(){
    List tags = widget.mediaGallery != null ? widget.mediaGallery[0]['tags'] : widget.mediaObj["tags"];
    return Container(
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      userIcon(context, widget.senderId, sendBy, profileImg, anonImg, widget.anon, blocked),
                      SizedBox(height: 5,),
                      tags != null && tags.isNotEmpty && tags[0].isNotEmpty ?
                      hashTags(tags:tags, boxColor: Colors.black54, borderColor: Colors.transparent, textColor: Colors.orange) :
                      SizedBox.shrink()
                    ],
                  ),
                )
  ...
  getChatSendTime() async{
    if(widget.groupId != null){
      DocumentSnapshot chatDS = await DatabaseMethods().groupChatCollection
          .doc(widget.groupId)
          .collection('chats')
          .doc(widget.mediaId)
          .get();

      if(chatDS.exists) sendTime = chatDS.data()['time'];
    }
  }
  ...
  void dispose() {
    ...
    unbindBackgroundIsolate();
    if(commentEditingController != null)
      commentEditingController.dispose();
    if(replyEditingController != null)
  ...
  void initState() {
    ...
    if(widget.showInfo){
      if(widget.story==null || !widget.story){
        ...
        replyEditingController = new TextEditingController();
        commentEditingController = new TextEditingController();
        DatabaseMethods(uid: Constants.myUserId).markSeenStory(widget.senderId, widget.mediaId);
        getStoryComments();
        checkStoryExist();
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.mediaObj != null)
      mediaUrl = widget.mediaObj['imgUrl'] != null ? widget.mediaObj['imgUrl'] :
      widget.mediaObj['fileUrl'] != null ? widget.mediaObj['fileUrl'] : null;

    bool video = widget.mediaObj != null &&
        widget.mediaObj["imgName"] != null &&
        videoChecker(widget.mediaObj["imgName"]);

    bool mature = widget.mediaObj != null && widget.mediaObj['mature'] != null && widget.mediaObj['mature'];

    String fileName = widget.mediaObj != null && widget.mediaObj["fileName"] != null ?
    widget.mediaObj["fileName"] : null;

    String audioName = fileName != null && audioChecker(fileName) ? fileName : null;
    String pdfName = fileName != null && pdfChecker(fileName) ? fileName : null;

    String caption = widget.mediaObj != null ? widget.mediaObj['caption'] : null;
    String link = widget.mediaObj != null ? widget.mediaObj['link'] : null;

    List<Widget> gifyStickers = widget.mediaObj != null && widget.mediaObj['gifs'] != null ?
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          leading: BackButton(
            color: audioName == null && pdfName == null ? Colors.white : Colors.black,
          ),
          title: status == DownloadTaskStatus.running ? LinearProgressIndicator(value:progress/100) : null,
          backgroundColor: audioName == null && pdfName == null ? Colors.transparent : Colors.white,
          elevation: 0,
          actions: audioName != null || pdfName != null ? [
            IconButton(
              icon: Icon(
                status == DownloadTaskStatus.undefined || status == DownloadTaskStatus.canceled ?
                platform == TargetPlatform.android ?
                Icons.download_rounded :
                CupertinoIcons.download_circle :
                status == DownloadTaskStatus.running ? Icons.close :
                status == DownloadTaskStatus.complete ? Icons.open_in_new_rounded :
                status == DownloadTaskStatus.failed ? Icons.refresh : null,
                color: Colors.black,
              ),
              onPressed: (){
                if(status == DownloadTaskStatus.undefined || status == DownloadTaskStatus.canceled){
                  String fileName = audioName != null ? audioName : pdfName;
          ...
          children: [
            Stack(
                ...
                children:[
                  ...
                  widget.showInfo ?
                  StreamBuilder(
                    stream: DatabaseMethods().userCollection.doc(widget.senderId).snapshots(),
                    builder: (context, snapshot) {
                      if(snapshot.hasData && snapshot.data.data() != null){
                        ...
                        blocked = val.data()["blockedBy"] != null && val.data()["blockedBy"].contains(Constants.myUserId);
        ...
        Column(
          ...
          children: [
            ...
            commentBtt(
                ...
                context,
                widget.senderId,
                widget.mediaObj != null && widget.mediaObj["imgName"] != null ? widget.mediaObj : null,
                widget.mediaObj != null && widget.mediaObj["fileName"] != null ? widget.mediaObj : null,
                widget.mediaId,
                widget.groupId,
                widget.mediaGallery,
                widget.anon
            ...
            moreOpsBtt(
                ...
                context:context,
                imgObj:widget.mediaObj != null && widget.mediaObj["imgName"] != null ? widget.mediaObj : null,
                fileObj:widget.mediaObj != null && widget.mediaObj["fileName"] != null ? widget.mediaObj : null,
                mediaGallery:widget.mediaGallery,
                senderId:widget.senderId,
                groupId:widget.groupId,
                mediaId:widget.mediaId,
      ...
      child: Container(
        ...
        child:Column(
          ...
          children: [
            ...
            Image.asset("assets/icon/nsfwIcon.png", scale: 2.5,),
    ...
    return Stack(
      children: [
        ...
        VideoAudioUrlPreview(
          ...
          muteBttPadding: widget.video ? EdgeInsets.fromLTRB(0.0, MediaQuery.of(context).size.height*0.125, 9.0, 0.0) : null,
          gifs: widget.gifyStickers,
          caption: widget.caption,
          link: widget.link,
          displayGifs: true,
